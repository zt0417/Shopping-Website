...
!(function($) {
    var picScroll = function() {
        var arg = arguments,
        defaults = {// css{盒子的宽高};config{每次滑动/淡进淡出间隔时间time、滑动类型("top/left/fade")、滑动/淡进淡出的速度speed、是否� 载左右按钮button}。注：如不自定义参数则采用默认值
            css: {"width": 627, "height": 250},
            ...
            animates = {
                ...
                config: function(str) {
                    ...
                    if (con.type == "left" || con.type == "top") {//动画类型判断
                        ...
                        if (that.list == that.li.length) {//如果当前图片是第一� 从最后循环
                            con.type == "top" ? that.li.first().css(arr[0], that.ul.height()) : that.li.first().css(arr[0], that.ul.width());//给第一� 图片的position: relative;赋值以达到� 限循环效果
                        ...
                        if (that.list == -1) {//如果当前图片是最后一� 从第一� 循环
                            con.type == "top" ? that.li.last().css(arr[0], -that.ul.height()) : that.li.last().css(arr[0], -that.ul.width());//给最后� 图片的position: relative;赋值以达到� 限循环效果
                        ...
                        if (that.list == that.li.length) {//如果为最后一� 图
                            that.list = 0;
                        }
                    ...
                    for(;i<butArr.length;i++){
                        $(butArr[i]).eq(that.list == that.li.length ? 0 : that.list).siblings().removeClass("on");//按钮� �式变化
                        $(butArr[i]).eq(that.list == that.li.length ? 0 : that.list).addClass("on");//按钮� �式变化
                    }
                },
